...
async function load(){
  ...
  const cards={
    ...
    'LiveChat':String(s.livechatHealth?.status||integGroup(['livechat_poll','livechat_deep_sync','livechat'],s.livechatConfigured)),
    'Telegram':s.humanBridge?.configured?(s.humanBridge?.enabled?integ('telegram_poll',true):'WARNING'):'WARNING',
    'OpenAI':integ('openai',s.openaiConfigured)
  };
  ...
  $('#healthDetails').innerHTML=`<pre>${esc(JSON.stringify({poller:s.poller,livechatIngress:s.livechatIngress,learningWorker:s.learningWorker,cannedSync:s.cannedSync,humanBridge:s.humanBridge,livechatHealth:s.livechatHealth,integrationHealth:s.integrationHealth,operations:o},null,2))}</pre>`
}
