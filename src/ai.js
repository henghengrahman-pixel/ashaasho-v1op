...
export class OpenAIClient {
  ...
  async completeVisionStructured(system,text,attachments=[],options={}){
    ...
    try{
      ...
      const status=Number(e?.status||String(e?.message||'').match(/OPENAI_(\d{3})/)?.[1]||0);
      if(![400,404,405,415,422].includes(status)) throw e;
      // Some endpoint/model combinations can understand images but do not accept strict
      // structured output in the same multimodal request. Never use a free-form JSON
      // answer as the decision. Stage 1 extracts controlled visible facts; stage 2 feeds
      // those observations through the normal strict JSON Schema decision endpoint.
      const observationSystem='Inspect the attached customer-service image(s). Return a concise factual observation only: visible text, visible transaction/account fields, error messages, and whether the image is unreadable. Do not make a customer-service decision, do not infer success/failure beyond visible evidence, and do not output JSON.';
      const observed=await this.completeVision(observationSystem,text,attachments);
      const staged=`ORIGINAL CUSTOMER TEXT:
${String(text||'').slice(0,8000)}

CONTROLLED IMAGE OBSERVATION:
${String(observed?.text||'').slice(0,6000)}`;
      const structured=await this.completeStructured(system,[{role:'user',content:staged}],{...options,schema,name,maxOutputTokens:structuredMax});
      return {...structured,visionFallback:'two_stage_structured',visionUsage:observed?.usage||null};
    }
  }
...
${String(history||'').slice(-12000)}`;
  ...
  async decideAgent({input={},style={}}={}) {
    ...
    let repairAttempted=false,structuredValid=false,telemetryResult=null;
    const emitTelemetry=(result,{valid=false,fallback=false}={})=>{
      const raw=result?.raw||{}; const finishReason=raw?.choices?.[0]?.finish_reason||raw?.status||raw?.incomplete_details?.reason||null;
      const usage=result?.usage||null; const truncation=finishReason==='length'||finishReason==='max_output_tokens'||Boolean(raw?.incomplete_details);
      console.info(JSON.stringify({event:'openai_structured_decision',model:config.openaiModel,endpointMode:config.openaiStyle,finishReason,outputType:result?.visionFallback||'json_schema',structuredValidation:Boolean(valid),repairAttempted,latencyMs:Date.now()-startedAt,tokenUsage:usage||null,truncationIndicator:Boolean(truncation),safeFallback:Boolean(fallback)}));
    };
...
${JSON.stringify(context,null,2)}`,input.attachments||[],{schema:AGENT_DECISION_SCHEMA,name:'conversation_decision'});
    ...
    let parsed=null;
    try{parsed=validateAgentShape(locallyRepairAgentCandidate(parseJsonLoose(result.text),input));structuredValid=true;}
    catch(firstError){
      repairAttempted=true;
      try{const repaired=await this.repairAgentStructured(result.text,system);parsed=validateAgentShape(locallyRepairAgentCandidate(parseJsonLoose(repaired.text),input));result={...repaired,usage:repaired.usage||result.usage};telemetryResult=result;structuredValid=true;}
      catch{emitTelemetry(telemetryResult||result,{valid:false,fallback:true});return safeAgentFallback(input,state,result.usage);}
    }
    emitTelemetry(telemetryResult||result,{valid:structuredValid,fallback:false});
    return {
