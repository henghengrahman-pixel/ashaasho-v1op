...
import { evaluateAiOutboundState } from './outbound-gate.js';
import { sendHumanBridgeWithControlledClaim } from './human-bridge-delivery.js';
...
async function resolveContextualIntent(chatId,currentIntent,text,attachments=[]){
  ...
  if(workflowIntent && explicitOperational.has(fresh) && fresh!==workflowIntent){
    await db.appendCaseAudit({chatId,eventType:'OPERATIONAL_TOPIC_SWITCH',intent:fresh,payload:{from:workflowIntent,to:fresh,workflowType:wfType,workflowState:wf?.workflow_state||null,text:String(text||'').slice(0,240)}}).catch(()=>{});
...
function isLiveChatMembershipRequiredError(err){
  if(!err)return false;
  ...
  return err.cause ? isLiveChatMembershipRequiredError(err.cause) : false;
}

async function finalAiOutboundGate(chatId,senderType='ai'){
  if(senderType!=='ai') return {allowed:true,reason:null,lane:null};
  const state=await db.getConversationState(chatId);
  return evaluateAiOutboundState(state);
}
...
async function sendAndStore(livechat,chatId,text,intent,senderType='ai'){
  ...
  try{
    ...
    for(let attempt=1;attempt<=maxAttempts;attempt++){
      try{
        // Final provider-send gate. The conversation can change lane or be taken over
        // while AI/idempotency work is in flight. Re-read the authoritative local state
        // immediately before every provider send attempt; never let stale MY_CHAT state
        // reach send_event. Human-bridge sends intentionally keep their existing policy.
        const finalGate=await finalAiOutboundGate(chatId,senderType);
        if(!finalGate.allowed){
          if(idempotencyKey) await db.failOutbound(idempotencyKey,`AI_OUTBOUND_SUPPRESSED:${finalGate.reason}`).catch(()=>{});
          await db.appendCaseAudit({chatId,eventType:'AI_OUTBOUND_SUPPRESSED_STATE_CHANGE',intent,payload:{senderType,sourceEventId,reason:finalGate.reason,lane:finalGate.lane,attempt}}).catch(()=>{});
          return {suppressed:true,reason:String(finalGate.reason||'AI_OUTBOUND_GATE_CLOSED').toLowerCase(),lane:finalGate.lane||null};
        }
        if(senderType==='human_bridge'){
          const delivered=await sendHumanBridgeWithControlledClaim({
            livechat,
            chatId,
            send:()=>livechat.sendMessage(chatId,text,customId?{customId}:undefined),
            onClaimed:async claimed=>{
              await db.setConversationLane(chatId,'MY_CHAT');
              await db.recordLiveChatClaimSuccess(chatId).catch(()=>{});
              await db.setIntegrationHealth('livechat_requester',{status:'OK',meta:{lastAction:'claimChat',source:'human_bridge_delivery',chatId,providerLane:String(claimed?.lane||'MY_CHAT').toUpperCase()}}).catch(()=>{});
              await db.appendCaseAudit({chatId,eventType:'HUMAN_BRIDGE_CONTROLLED_CLAIM_RECOVERED',intent,payload:{senderType,sourceEventId}}).catch(()=>{});
            }
          });
          sent=delivered.sent;
        }else{
        ...
        lastSendError=err;
        if(attempt>=maxAttempts || !isRetryableTurnError(err)) break;
    ...
    // A successful provider send proves the requester is currently allowed to send in this chat.
    // Clear any older global membership WARNING so health reflects the latest verified state.
    await db.setIntegrationHealth('livechat_membership',{status:'OK',latencyMs:Date.now()-started,meta:{lastAction:'sendMessage',chatId,senderType}}).catch(()=>{});
    ...
    const membershipRequired=isLiveChatMembershipRequiredError(err);
    if(inactive) await db.markConversationEnded(chatId,{reason:'SEND_CONFIRMED_INACTIVE'}).catch(()=>{});
    else if(publicAgentLimit || membershipRequired){
      await db.setConversationLane(chatId,'SUPERVISED').catch(()=>{});
      err.retryable=false;
      await db.setIntegrationHealth('livechat_membership',{status:'WARNING',latencyMs:Date.now()-started,error:String(err?.message||err),meta:{lastAction:'sendMessage',chatId,lane:'SUPERVISED',membershipMutationAttempted:false,reason:publicAgentLimit?'PUBLIC_AGENT_LIMIT':'REQUESTER_NOT_CHAT_MEMBER'}}).catch(()=>{});
      await db.appendCaseAudit({chatId,eventType:'PROVIDER_MEMBERSHIP_REQUIRED',intent,payload:{senderType,sourceEventId,reason:publicAgentLimit?'PUBLIC_AGENT_LIMIT':'REQUESTER_NOT_CHAT_MEMBER',membershipMutationAttempted:false}}).catch(()=>{});
    }else if(err?.retryable!==false) err.retryable=true; // only transient delivery/network/provider failures are retried
    if(!publicAgentLimit && !membershipRequired) await db.setIntegrationHealth('livechat',{status:'ERROR',latencyMs:Date.now()-started,error:String(err?.message||err),meta:{lastAction:'sendMessage'}}).catch(()=>{});
...
export async function processCustomerMessage({chatId,eventId,threadId=null,text,createdAt,livechat,attachments=[],skipInsert=false,resumeReason=''}) {
  ...
  const currentLane=String(beforeState?.lc_lane||'OTHER').toUpperCase();
  if(currentLane!=='MY_CHAT'){
    // A previous send may have discovered that provider membership changed while this
    // snapshot was already being processed. Persist the event, leave its processing
    // ledger pending, and stop before OpenAI/Telegram. A later successful explicit
    // Handle with AI will sync the same event id and resume it exactly once.
    if(!skipInsert){
      ...
      await db.registerCustomerEventProcessing(chatId,eventId,{threadId,sessionId:beforeState?.session_id,delayMs:0}).catch(()=>{});
    }
    return {skipped:`lane_${currentLane.toLowerCase()}`,intent,normalized};
  }
...
Silakan cek deposit member.`,requireTelegramDelivery:true});
      ...
      // Provider ownership changed after discovery. Do not create another ticket and do
      // not attempt a fallback send: both would hit the same 403. Leave the customer
      // event FAILED (outer ledger catch) so an explicit successful Handle with AI can
      // resume it later from the same event id without marking a response as DONE.
      if(isLiveChatMembershipRequiredError(e) || String(e?.code||'')==='LIVECHAT_PUBLIC_AGENT_LIMIT'){
        await db.appendCaseAudit({chatId,eventType:'AI_SEND_BLOCKED_BY_PROVIDER_MEMBERSHIP',intent,payload:{eventId,code:String(e?.code||''),retryable:false}}).catch(()=>{});
        e.retryable=false;
        throw e;
      }
