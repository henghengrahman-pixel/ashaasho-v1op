export function classifyCustomerEventError(error,attempts=1){
  ...
  if(/LIVECHAT_CHAT_INACTIVE|closed chat|session obsolete|INVALID_EVENT|INVALID_PROVIDER_IDENTITY/i.test(message)) return {errorClass:'PERMANENT',code,retryable:false,terminal:true,delayMs:0};
