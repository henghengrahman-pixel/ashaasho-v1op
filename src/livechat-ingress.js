...
export function isAutomaticClaimSource(source=''){
  return ['poll','poll_unchanged','deep_sync'].includes(text(source).toLowerCase());
}

export function shouldRunClaimJob(source='',{autoClaimEnabled=config.lcAutoClaimQueued}={}){
  return !isAutomaticClaimSource(source) || Boolean(autoClaimEnabled);
}
...
async function handleJob(job,livechat){
  ...
  if(type==='CLAIM_CHAT'){
    ...
    const claimSource=text(job?.payload?.source).toLowerCase();
    // Durable jobs can outlive a deployment. If an old poll/deep-sync auto-claim job was
    // queued before auto-claim was disabled, drain it without touching provider membership.
    // Explicit/manual claims are not routed through these automatic source labels.
    if(!shouldRunClaimJob(claimSource)) return {claimed:false,skipped:'AUTO_CLAIM_DISABLED',source:claimSource};
    ...
    try{
      const claimed=await livechat.claimChat(chatId,{ensureMembership:true,knownChat:job?.payload?.summary||null});
      const providerLane=String(claimed?.lane||'OTHER').toUpperCase();
      if(providerLane!=='MY_CHAT'){
        // Never leave a stale local MY_CHAT lane after provider verification says otherwise.
        // This closes the AI outbound gate immediately while the claim is blocked/cooling down.
        await db.setConversationLane(chatId,['QUEUED','SUPERVISED','OTHER'].includes(providerLane)?providerLane:'OTHER').catch(()=>{});
        try{await livechat.rollbackClaimFollow(chatId,claimed,{reason:'OWNERSHIP_NOT_CONFIRMED'});}
        catch(cleanupError){
          await db.logError('livechat_ingress','CLAIM_ROLLBACK_FAILED',cleanupError.message,{chatId,claimCleanup:cleanupError.claimCleanup||null}).catch(()=>{});
          throw cleanupError;
        }
        ...
        await db.setIntegrationHealth('livechat_requester',{status:'WARNING',error:`LIVECHAT_CLAIM_OWNERSHIP_NOT_CONFIRMED: provider lane is ${providerLane}`,meta:{lastAction:'claimChat',source:'ingress',chatId,code:'LIVECHAT_CLAIM_OWNERSHIP_NOT_CONFIRMED',providerLane,claimVerification:claimed?.claimVerification||null}}).catch(h=>console.warn(JSON.stringify({event:'livechat_requester_health_write_failed',chatId,error:h.message})));
      ...
      await db.setIntegrationHealth('livechat_requester',{status:'OK',meta:{lastAction:'claimChat',source:'ingress',chatId,providerLane}}).catch(h=>console.warn(JSON.stringify({event:'livechat_requester_health_write_failed',chatId,error:h.message})));
      await db.setIntegrationHealth('livechat_membership',{status:'OK',meta:{lastAction:'claimChat',source:'ingress',chatId,providerLane}}).catch(h=>console.warn(JSON.stringify({event:'livechat_membership_health_write_failed',chatId,error:h.message})));
      return await syncChatById(livechat,chatId,claimed.chat||{id:chatId});
    }catch(e){
      const code=String(e?.code||'');
      if(code==='LIVECHAT_PUBLIC_AGENT_LIMIT' || /PUBLIC_AGENT_LIMIT/i.test(String(e?.message||''))){
        ...
        await db.setIntegrationHealth('livechat_requester',{status:'WARNING',error:String(e?.message||e),meta:{lastAction:'claimChat',source:'ingress',chatId,code:'LIVECHAT_PUBLIC_AGENT_LIMIT',claimCleanup:e?.claimCleanup||null}}).catch(h=>console.warn(JSON.stringify({event:'livechat_requester_health_write_failed',chatId,error:h.message})));
        return {claimed:false,lane:'QUEUED',blocked:'PUBLIC_AGENT_LIMIT'};
      }
      if(['LIVECHAT_REQUESTER_USER_ID_REQUIRED','LIVECHAT_REQUESTER_USER_ID_INVALID','LIVECHAT_REQUESTER_USER_ID_INVALID_FORMAT'].includes(code)){
        await db.setIntegrationHealth('livechat_requester',{status:'ERROR',error:String(e?.message||e),meta:{lastAction:'claimChat',source:'ingress',chatId,code,claimCleanup:e?.claimCleanup||null}}).catch(h=>console.warn(JSON.stringify({event:'livechat_requester_health_write_failed',chatId,error:h.message})));
      }else{
        // Surface real claim failures instead of leaving requesterRuntimeStatus at NOT_VERIFIED.
        await db.setIntegrationHealth('livechat_requester',{status:'WARNING',error:String(e?.message||e),meta:{lastAction:'claimChat',source:'ingress',chatId,code:code||'LIVECHAT_CLAIM_FAILED',claimCleanup:e?.claimCleanup||null}}).catch(h=>console.warn(JSON.stringify({event:'livechat_requester_health_write_failed',chatId,error:h.message})));
      }
      throw e;
    }
    });
...
export async function liveChatIngressStatus(){
  const [q,events,ops]=await Promise.all([
    db.liveChatIngressStats().catch(()=>({pending:null,runnable_pending:null,delayed_pending:null,processing:null,done:null,cancelled:null,dead:null,retried_jobs:null,retry_attempts_total:null,oldest_pending_age_ms:null,oldest_runnable_age_ms:null})),
    db.customerEventProcessingStats().catch(()=>({pending:null,processing:null,done:null,failed:null,blocked:null,dead:null,runnable:null,oldest_pending_age_ms:null,oldest_runnable_age_ms:null})),
    db.runtimeOperationalStats().catch(()=>null)
  ]);
  const queueLag=Math.max(Number(q?.oldest_runnable_age_ms||0),Number(events?.oldest_runnable_age_ms||0));
  const queueHealth=queueLag>=config.lcQueueCriticalMs?'CRITICAL':queueLag>=config.lcQueueDegradedMs?'DEGRADED':'HEALTHY';
  return {running:Boolean(timer),tickRunning:running,inFlight,workerId,lastRunAt,lastSuccessAt,lastError,processedTotal,retriedTotal,deadTotal,queueHealth,queueLagMs:queueLag,queueThresholds:{degradedMs:config.lcQueueDegradedMs,criticalMs:config.lcQueueCriticalMs},queue:q,eventProcessing:events,operations:ops};
}
