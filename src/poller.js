...
export function pollerStatus(){ return {
  ...
  autoClaim:{enabled:Boolean(config.lcAutoClaimQueued),requested:Boolean(config.lcAutoClaimQueuedRequested),policyAck:Boolean(config.lcAutoClaimQueuedPolicyAck),batch:Number(config.lcAutoClaimQueuedBatch||0),cooldownSeconds:Number(config.lcClaimCooldownSeconds||0)},
...
// CLAIM_CHAT jobs need durable dedupe, but not permanent dedupe. A completed/blocked
// claim remains stored in livechat_ingress_jobs; using only chatId+fingerprint would make
// an unchanged QUEUED chat impossible to claim again. A short bucket suppresses duplicate
// hot/deep-sync enqueues while still allowing a later attempt after the database cooldown.
export function claimDedupeBucket(now=Date.now(),cooldownSeconds=config.lcClaimCooldownSeconds){
  // Match the durable producer dedupe window to the configured claim cooldown.
  // Older builds clamped this to 30s, causing repeated CLAIM_CHAT jobs while the
  // conversation itself was still blocked for 120s.
  const seconds=Math.max(30,Math.min(3600,Number(cooldownSeconds)||120));
  return Math.floor(Number(now)/(seconds*1000));
}

export function claimDedupeKey(chatId,fingerprint,now=Date.now()){
  return `claim:${String(chatId)}:${String(fingerprint)}:${claimDedupeBucket(now)}`;
}
...
export function summaryFingerprint(summary){
  // Fingerprint only provider state that can change processing semantics. Raw list_chats
  // summaries may contain volatile/ordering metadata; hashing the entire object created
  // a new durable SYNC_CHAT for semantically identical states. Event identity remains
  // separate and is never coalesced here.
  const thread=summary?.last_thread_summary||summary?.last_thread||summary?.thread||{};
  const users=Array.isArray(summary?.users)?summary.users.map(u=>({id:u?.id||u?.user_id||u?.email||null,type:u?.type||null})).filter(u=>u.id):[];
  const stable={
    id:summary?.id||null,
    status:summary?.status||null,
    state:summary?.state||null,
    active:typeof summary?.active==='boolean'?summary.active:null,
    is_followed:typeof summary?.is_followed==='boolean'?summary.is_followed:null,
    routing_status:summary?.routing_status||null,
    queue_position:Number.isFinite(Number(summary?.queue_position))?Number(summary.queue_position):null,
    thread:{id:thread?.id||thread?.thread_id||null,active:typeof thread?.active==='boolean'?thread.active:null,status:thread?.status||null,routing_status:thread?.routing_status||null,queue_position:Number.isFinite(Number(thread?.queue_position))?Number(thread.queue_position):null,total_events:Number.isFinite(Number(thread?.total_events))?Number(thread.total_events):null},
...
async function runDeepSync(livechat){
  ...
  try{
    ...
    for (const [rank, summary] of chats.entries()) {
      ...
      else if(lane==='QUEUED' && config.lcAutoClaimQueued && autoClaimQueued<config.lcAutoClaimQueuedBatch){const claim=await db.enqueueLiveChatIngressJob({dedupeKey:claimDedupeKey(chatId,fp),jobType:'CLAIM_CHAT',chatId,payload:{summary,source:'deep_sync'},priority:95}).catch(()=>null);if(claim)autoClaimQueued++;}
    }
    laneCounts=counts;
      ...
      try{
        ...
        if(oldFp===fp && oldLane===lane && oldRank===rank){
          if(lane==='QUEUED' && config.lcAutoClaimQueued && autoClaimQueued<config.lcAutoClaimQueuedBatch){
            const claim=await db.enqueueLiveChatIngressJob({dedupeKey:claimDedupeKey(chatId,fp),jobType:'CLAIM_CHAT',chatId,payload:{summary,source:'poll_unchanged'},priority:95});
        ...
        if(lane==='MY_CHAT' || lane==='OTHER'){
          ...
          const claim=await db.enqueueLiveChatIngressJob({dedupeKey:claimDedupeKey(chatId,fp),jobType:'CLAIM_CHAT',chatId,payload:{summary,source:'poll'},priority:95});
