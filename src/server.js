...
const appVersion=String(JSON.parse(readFileSync(new URL('../package.json',import.meta.url),'utf8')).version||'unknown');
const app=express(); const lc=new LiveChatClient(); const ai=new OpenAIClient();

function liveChatHealthSnapshot(integrationHealth=[]){
  const rows=Array.isArray(integrationHealth)?integrationHealth:[];
  const byName=new Map(rows.map(row=>[String(row?.integration||'').toLowerCase(),row]));
  const requester=lc.requesterIdentityStatus();
  const connectionRows=['livechat_poll','livechat_deep_sync','livechat'].map(name=>byName.get(name)).filter(Boolean);
  const connectionStatus=connectionRows.some(row=>String(row?.status||'').toUpperCase()==='ERROR')?'ERROR':(connectionRows.some(row=>String(row?.status||'').toUpperCase()==='OK')?'OK':'NOT_VERIFIED');
  const requesterRow=byName.get('livechat_requester');
  const requesterRuntimeStatus=requesterRow?String(requesterRow.status||'').toUpperCase():'NOT_VERIFIED';
  let status='WARNING';
  if(!lc.ready() || !requester.explicitConfigured || !requester.formatValid || connectionStatus==='ERROR' || requesterRuntimeStatus==='ERROR') status='ERROR';
  else if(connectionStatus==='OK' && requesterRuntimeStatus==='OK') status='HEALTHY';
  return {
    status,
    credentialsConfigured:lc.ready(),
    connectionStatus,
    connectionValid:connectionStatus==='OK',
    requesterConfigured:requester.explicitConfigured,
    requesterAvailable:requester.available,
    requesterFormatValid:requester.formatValid,
    requesterSource:requester.source,
    requesterRuntimeStatus,
    requesterRuntimeVerified:requesterRuntimeStatus==='OK',
    claimConfigured:Boolean(lc.ready()&&requester.explicitConfigured&&requester.formatValid)
  };
}
...
async function closeConversationFromInactiveProvider(chatId,error,reason='LIVECHAT_CONFIRMED_INACTIVE'){
  ...
  try{
    await markConversationEnded(chatId,{reason});
    forgetChat(chatId);
    return true;
  }catch(persistError){
    await logError('conversation','INACTIVE_CLOSE_PERSIST_FAILED',persistError.message,{chatId,reason}).catch(logErrorFailure=>console.error(JSON.stringify({event:'inactive_close_log_failed',chatId,error:logErrorFailure.message})));
    const e=new Error(`LIVECHAT_INACTIVE_CLOSE_PERSIST_FAILED: ${String(persistError?.message||persistError)}`);
    e.code='LIVECHAT_INACTIVE_CLOSE_PERSIST_FAILED'; e.status=503; e.cause=persistError;
    throw e;
  }
}
...
async function claimDashboardChat(chatId){
  ...
  try{
    ...
    const claimed=await lc.claimChat(id,{ensureMembership:true,knownChat:before});
    ...
    if(providerLane!=='MY_CHAT' && confirmedLane!=='MY_CHAT'){
      try{await lc.rollbackClaimFollow(id,claimed,{reason:'OWNERSHIP_NOT_CONFIRMED'});}
      catch(cleanupError){
        await logError('livechat','CLAIM_ROLLBACK_FAILED',cleanupError.message,{chatId:id,claimCleanup:cleanupError.claimCleanup||null});
        throw cleanupError;
      }
    ...
    await setIntegrationHealth('livechat_requester',{status:'OK',meta:{lastAction:'claimChat',source:'dashboard',chatId:id}}).catch(h=>console.warn(JSON.stringify({event:'livechat_requester_health_write_failed',error:h.message})));
    return {ok:true,lane:'MY_CHAT',providerLane:confirmedLane,membership:claimed.membership||null,claimMutation:claimed.claimMutation||null};
  }catch(e){
    const code=String(e?.code||'');
    const publicLimit=code==='LIVECHAT_PUBLIC_AGENT_LIMIT'||/PUBLIC_AGENT_LIMIT/i.test(String(e?.message||''));
    const requesterConfigError=['LIVECHAT_REQUESTER_USER_ID_REQUIRED','LIVECHAT_REQUESTER_USER_ID_INVALID','LIVECHAT_REQUESTER_USER_ID_INVALID_FORMAT'].includes(code);
    ...
    if(publicLimit||requesterConfigError){
      await setIntegrationHealth('livechat_requester',{status:requesterConfigError?'ERROR':'WARNING',error:String(e?.message||e),meta:{lastAction:'claimChat',source:'dashboard',chatId:id,code,claimCleanup:e?.claimCleanup||null}}).catch(h=>console.warn(JSON.stringify({event:'livechat_requester_health_write_failed',error:h.message})));
    }
    throw e;
  }
}
...
async function boot(){
  try{
    ...
    const ingressRecovery=await withPostgresStartupRetry('ingress-backlog-recovery',()=>recoverLiveChatIngressBacklog());
    console.log(JSON.stringify({event:'livechat_ingress_backlog_recovery',...ingressRecovery}));
