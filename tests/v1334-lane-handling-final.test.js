...
test('v1.33.4 queued claim uses follow + membership + detail and becomes claimable',async()=>{
  ...
  const out=await lc.claimChat('q1',{knownChat:{id:'q1',is_followed:false,last_thread_summary:{active:true}}});
...
test('v1.33.4 public-agent-limit rolls back follow so chat cannot masquerade as MY_CHAT',async()=>{
  ...
  await assert.rejects(()=>lc.claimChat('q2',{knownChat:{id:'q2',is_followed:false,last_thread_summary:{active:true}}}),/LIVECHAT_PUBLIC_AGENT_LIMIT/);
