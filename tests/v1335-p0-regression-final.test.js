...
test('06 queued uses controlled auto claim by default',()=>assert.match(cfg,/LIVECHAT_AUTO_CLAIM_QUEUED, true/));
test('07 queued lane remains metadata-only until controlled claim promotes it',()=>{assert.match(poller,/QUEUED\/SUPERVISED stay visible as lightweight inventory lanes/);assert.match(poller,/config\.lcAutoClaimQueued/)});
